...
- e773992: **Breaking:** Removed `Fetch` from the public API of `mppx/client`. `Mppx.create()` now returns a `fetch` function and polyfills `globalThis.fetch` by default.
...
- df400e3: Added composed `tempo()` client factory that returns both `charge` and `stream` method intents.
  ...
  `tempo.charge()` and `tempo.stream()` continue to work for individual use. `Mppx.create` now accepts nested tuples in `methods` and flattens them automatically.
...
- aea84a3: Supported nested method tuples in server `Mppx.create`, matching the client-side `Methods` pattern. Methods are automatically flattened via `FlattenMethods`.
