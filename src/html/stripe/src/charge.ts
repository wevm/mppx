...
mount<typeof Methods.charge, charge.HtmlConfig, RequiredBy<charge.HtmlActions, 'createToken'>>(
  async (c) => {
    ...
    const createTokenUrl = c.actions.createToken
    ...
    let pendingAppearanceUpdate = 0
    let previousAppearance = JSON.stringify(initialAppearance)

    function syncAppearance() {
      if (pendingAppearanceUpdate) return
      pendingAppearanceUpdate = requestAnimationFrame(() => {
        pendingAppearanceUpdate = 0
        const nextAppearance = getAppearance()
        const nextAppearanceKey = JSON.stringify(nextAppearance)
        if (nextAppearanceKey === previousAppearance) return
        previousAppearance = nextAppearanceKey
        elements.update({ appearance: nextAppearance })
      })
    }

    window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', syncAppearance)

    // Coalesce theme-driven CSS var updates into a single Stripe appearance refresh.
