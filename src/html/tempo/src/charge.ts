...
import { charge as tempoCharge } from '../../../tempo/client/Charge.js'
...
mount<typeof Methods.charge, charge.HtmlConfig>((c) => {
  ...
  async function pay() {
    ...
    try {
      ...
      const credential = await method.createCredential({ challenge: c.challenge })
      dispatchEvent(new CustomEvent('mppx:complete', { detail: credential }))
    } catch (error) {
      const message =
