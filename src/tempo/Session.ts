...
async function createOpenCredential(
  ...
  const { signer, voucherSigner } = parameters
  ...
  if ('channelId' in parameters) {
    const chainId = resolveChainId(client, parameters.challenge)
    const escrowContract = resolveEscrow(parameters.challenge, chainId, parameters.escrowContract)
    const payloadAuthorizedSigner = parameters.authorizedSigner ?? authorizedSigner
    if (authorizedSigner.toLowerCase() !== payloadAuthorizedSigner.toLowerCase())
      throw new Error('voucherSigner does not match authorizedSigner.')

    const signature = await signVoucher(
      client,
      signer,
      { channelId: parameters.channelId, cumulativeAmount: parameters.cumulativeAmount },
      escrowContract,
      chainId,
    ...
    return serializeCredential({
      ...
      payload: {
        action: 'open',
        type: 'transaction',
        authorizedSigner: payloadAuthorizedSigner,
  ...
  const transaction = parameters.transaction
    ...
    : await signSessionTransaction(client, {
        calls: filled.calls,
...
async function createTopUpCredential(
  ...
  const transaction =
    'transaction' in parameters && parameters.transaction
      ? parameters.transaction
      : await signSessionTransaction(client, {
          calls: filled.calls,
          feePayer: filled.feePayer,
