...
describe('precompile Channel.transactionForExpiringNonceHash', () => {
  test('returns transaction unchanged when no fee payer is used', () => {
    expect(Channel.transactionForExpiringNonceHash({ transaction })).toBe(transaction)
  })

  test('strips fee-payer signature from the hash preimage transaction', () => {
    expect(
