...
import { type OnChainChannel, getOnChainChannel, verifyTopUpTransaction } from '../Chain.js'
	...
	return Method.toServer(streamMethod, {
		...
		async verify({ credential }) {
			...
			switch (payload.action) {
				...
				case 'close':
					streamReceipt = await handleClose(storage, rpcUrl, walletClient, challenge, payload, methodDetails)
					break
...
async function verifyAndAcceptVoucher(parameters: {
	...
	const { storage, minVoucherDelta, challenge, channel, channelId, voucher, onChainDeposit, methodDetails } =
		parameters
