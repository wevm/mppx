...
import type { ChannelStorage, ChannelState, SessionState } from '../Storage.js'
...
vi.mock('../Chain.js', () => {
	...
	return {
		...
		verifyTopUpTransaction: vi.fn(async (_rpcUrl: string, _escrow: Address, _channelId: Hex, _txHash: Hex, previousDeposit: bigint) => {
			if (mockOnChainChannel.finalized) throw new Error('Channel is finalized on-chain')
			if (mockOnChainChannel.deposit <= previousDeposit) throw new Error('Channel deposit did not increase')
...
async function signTestVoucher(amount: bigint) {
	return signVoucher(walletClient, account, { channelId, cumulativeAmount: amount }, escrowContract, chainId)
}
...
describe('stream server Method', () => {
	...
	describe('open', () => {
		...
		test('rejects open when payee mismatch', async () => {
			__setMockChannel(makeOnChainChannel({ payee: '0x0000000000000000000000000000000000000099' as Address }))
			const server = createServer()
		...
		test('open -> voucher -> voucher -> close', async () => {
			...
			await server.verify({
				credential: {
					...
					payload: { action: 'open' as const, type: 'hash' as const, channelId, cumulativeAmount: '1000000', voucherSignature: await signTestVoucher(1000000n) },
				},
				request: makeRequest(),
			})
			...
			const r2 = await server.verify({
				credential: {
					...
					payload: { action: 'voucher' as const, channelId, cumulativeAmount: '3000000', signature: await signTestVoucher(3000000n) },
				},
				request: makeRequest(),
			})
			...
			const r3 = await server.verify({
				credential: {
					...
					payload: { action: 'voucher' as const, channelId, cumulativeAmount: '7000000', signature: await signTestVoucher(7000000n) },
				},
				request: makeRequest(),
			})
			...
			const r4 = await server.verify({
				credential: {
					...
					payload: { action: 'close' as const, channelId, cumulativeAmount: '7000000', voucherSignature: await signTestVoucher(7000000n) },
				},
				request: makeRequest(),
			})
